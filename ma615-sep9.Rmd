...
cty_mean <- round(cty_mean, 2)                      ## round the results to 2 decimal places
hwy_mean <- round(hwy_mean, 2)

tbl_1 <- cbind(class, cty_mean,hwy_mean)      ## form an array for the table

kable(tbl_1, digits = 2,                                 ## call kable to make the table
...
colnames(tbl_2) <- c('cMod3l 99', 'cMilage 99', 
                     "hmodel 99", "hmilage 99",
                     'cModel 08', 'cMilage 08',
                     "hmodel 08", "hmilage 08"
                     )

kable(tbl_2, digits = 2, booktabs = TRUE, caption = "Top 3 MPG Models") #%>% kable%>%add_header_above(c("City 1999" = 2,
                                                                                           #                            "Highway 1999" = 2,
                                                                                            #                           "City 2008" = 2,
                                                                                             #                          "Highway 2008" = 2))
...
ggplot(mpg) +
 aes(x = displ, y = cty, colour = class, size = displ) +
 geom_point() +
 scale_color_hue() +
 theme_minimal() +
...
ggplot(mpg) +
 aes(x = displ, y = hwy, colour = class, size = displ) +
 geom_point() +
 scale_color_hue() +
 theme_minimal() +
 facet_wrap(vars(year))
